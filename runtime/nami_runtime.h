...
#define NAMI_NULL                                                              \
  ((nami_value_t){.type = NAMI_TYPE_NULL, .value = {.as_int = 0}})
#define NAMI_TRUE                                                              \
  ((nami_value_t){.type = NAMI_TYPE_BOOL, .value = {.as_bool = true}})
#define NAMI_FALSE                                                             \
  ((nami_value_t){.type = NAMI_TYPE_BOOL, .value = {.as_bool = false}})
...
static inline nami_value_t nami_value_function(void *f) {
  nami_value_t val;
  val.type = NAMI_TYPE_FUNCTION;
  val.value.as_function = f;
  return val;
}
...
static inline nami_value_t
nami_value_string(const char *s); // Forward declaration
...
static inline nami_string_t *nami_string_concat(nami_string_t *a,
                                                nami_string_t *b) {
  size_t total = a->length + b->length;
  nami_string_t *result =
      (nami_string_t *)malloc(sizeof(nami_string_t) + total + 1);
  result->length = total;
...
static inline nami_string_t *nami_string_substring(nami_string_t *str,
                                                   int64_t start, int64_t end) {
  if (start < 0)
    start = 0;
  if (end > (int64_t)str->length)
    end = (int64_t)str->length;
  if (start >= end)
    return nami_string_create("");
  size_t len = (size_t)(end - start);
  nami_string_t *result =
      (nami_string_t *)malloc(sizeof(nami_string_t) + len + 1);
  result->length = len;
...
static inline int64_t nami_string_index_of(nami_string_t *str,
                                           nami_string_t *search) {
...
static inline nami_string_t *nami_string_replace(nami_string_t *str,
                                                 nami_string_t *search,
                                                 nami_string_t *replacement) {
  ...
  nami_string_t *result =
      (nami_string_t *)malloc(sizeof(nami_string_t) + new_len + 1);
  ...
  memcpy(result->data + idx + replacement->length,
         str->data + idx + search->length,
         str->length - (size_t)idx - search->length + 1);
  return result;
}
...
static inline nami_array_t *nami_string_split(nami_string_t *str,
                                              nami_string_t *delimiter) {
  nami_array_t *result = nami_array_create();

  if (delimiter->length == 0) {
    // Empty delimiter: split into individual characters
    for (size_t i = 0; i < str->length; i++) {
      nami_string_t *char_str =
          (nami_string_t *)malloc(sizeof(nami_string_t) + 2);
      char_str->length = 1;
      char_str->capacity = 2;
  ...
  while (current < end) {
    ...
    if (found == NULL) {
      ...
      nami_string_t *part =
          (nami_string_t *)malloc(sizeof(nami_string_t) + remaining + 1);
    ...
    nami_string_t *part =
        (nami_string_t *)malloc(sizeof(nami_string_t) + part_len + 1);
  ...
  if (current == end && str->length > 0) {
    ...
    if (last_delim >= str->data &&
        strncmp(last_delim, delimiter->data, delimiter->length) == 0) {
      nami_value_t val;
      val.type = NAMI_TYPE_STRING;
...
static inline nami_string_t *nami_string_join(nami_array_t *arr,
                                              nami_string_t *separator) {
  if (arr->length == 0) {
    return nami_string_create("");
  }
  ...
  nami_string_t *result =
      (nami_string_t *)malloc(sizeof(nami_string_t) + total_len + 1);
  ...
  for (int64_t i = 0; i < arr->length; i++) {
    nami_value_t item = arr->items[i];
    if (item.type == NAMI_TYPE_STRING && item.value.as_string != NULL) {
      memcpy(result->data + pos, item.value.as_string->data,
             item.value.as_string->length);
...
static inline void nami_array_push(nami_array_t *arr, nami_value_t value) {
  if (arr->length >= arr->capacity) {
    arr->capacity *= 2;
    arr->items = (nami_value_t *)realloc(arr->items, sizeof(nami_value_t) *
                                                         (size_t)arr->capacity);
  }
...
static inline void nami_array_set(nami_array_t *arr, int64_t index,
                                  nami_value_t value) {
  if (index >= 0 && index < arr->length) {
...
static inline nami_array_t *nami_array_slice(nami_array_t *arr, int64_t start,
                                             int64_t end) {
  if (start < 0)
    start = 0;
  if (end > arr->length)
...
static inline nami_array_t *nami_array_map(nami_array_t *arr,
                                           nami_function_t fn) {
  nami_array_t *result = nami_array_create();
  for (int64_t i = 0; i < arr->length; i++) {
...
static inline nami_array_t *nami_array_filter(nami_array_t *arr,
                                              nami_function_t fn) {
  nami_array_t *result = nami_array_create();
  for (int64_t i = 0; i < arr->length; i++) {
...
static inline nami_value_t nami_array_reduce(nami_array_t *arr,
                                             nami_function2_t fn,
                                             nami_value_t initial) {
...
static inline void nami_object_set(nami_object_t *obj, const char *key,
                                   nami_value_t value) {
  ...
  if (obj->count >= obj->capacity) {
    obj->capacity *= 2;
    obj->entries = (nami_object_entry_t *)realloc(
        obj->entries, sizeof(nami_object_entry_t) * (size_t)obj->capacity);
  }
...
static inline nami_value_t nami_object_get(nami_object_t *obj,
                                           const char *key) {
  for (int64_t i = 0; i < obj->count; i++) {
    if (strcmp(obj->entries[i].key, key) == 0) {
...
static inline void *nami_gc_alloc(nami_gc_t *gc, size_t size,
                                  nami_type_tag_t type) {
...
static inline void nami_gc_disable(nami_gc_t *gc) { gc->enabled = false; }

static inline void nami_gc_enable(nami_gc_t *gc) { gc->enabled = true; }
...
static nami_loop_guard_t __nami_default_loop_guard = {.iteration_count = 0,
...
static inline void nami_loop_guard_init(nami_loop_guard_t *guard,
...
static inline bool nami_truthy(nami_value_t v) {
  ...
  case NAMI_TYPE_INT:
    return v.value.as_int != 0;
  case NAMI_TYPE_FLOAT:
    return v.value.as_float != 0.0;
  case NAMI_TYPE_STRING:
    return v.value.as_string != NULL && v.value.as_string->length > 0;
  case NAMI_TYPE_NULL:
    return false;
...
static inline double nami_to_number(nami_value_t v) {
  switch (v.type) {
  case NAMI_TYPE_INT:
    return (double)v.value.as_int;
  case NAMI_TYPE_FLOAT:
    return v.value.as_float;
  case NAMI_TYPE_BOOL:
    return v.value.as_bool ? 1.0 : 0.0;
  case NAMI_TYPE_STRING:
    return atof(v.value.as_string->data);
  default:
    return 0.0;
  }
}
...
static inline nami_value_t nami_add(nami_value_t a, nami_value_t b) {
  ...
  if (a.type == NAMI_TYPE_STRING || b.type == NAMI_TYPE_STRING) {
    ...
    if (a.type == NAMI_TYPE_STRING && b.type == NAMI_TYPE_STRING) {
      ...
      val.type = NAMI_TYPE_STRING;
      val.value.as_string =
          nami_string_concat(a.value.as_string, b.value.as_string);
      return val;
    }
  }
  // Int + Int
...
static inline nami_value_t nami_div(nami_value_t a, nami_value_t b) {
  ...
  if (a.type == NAMI_TYPE_INT && b.type == NAMI_TYPE_INT &&
      a.value.as_int % b.value.as_int == 0) {
    return nami_value_int(a.value.as_int / b.value.as_int);
  }
...
static inline nami_value_t nami_neg(nami_value_t a) {
  if (a.type == NAMI_TYPE_INT)
    return nami_value_int(-a.value.as_int);
...
static inline bool nami_equals_raw(nami_value_t a, nami_value_t b) {
  ...
  case NAMI_TYPE_FLOAT:
    return a.value.as_float == b.value.as_float;
  case NAMI_TYPE_BOOL:
 ...
 * Requirements: 2.2, 2.4
 */
static inline nami_value_t nami_input_line(void) { return nami_input(); }
...
static inline nami_graph_t *nami_graph_create(int64_t num_vertices) {
  ...
  graph->adjacency_list =
      (nami_array_t **)malloc(sizeof(nami_array_t *) * (size_t)num_vertices);
 ...
 */
static inline void nami_graph_add_edge(nami_graph_t *graph, int64_t from,
                                       int64_t to, double weight) {
  if (from < 0 || from >= graph->num_vertices || to < 0 ||
 ...
 */
static inline nami_object_t *nami_graph_dijkstra(nami_graph_t *graph,
                                                 int64_t start, int64_t end) {
  if (start < 0 || start >= graph->num_vertices || end < 0 ||
  ...
  double *distances =
      (double *)malloc(sizeof(double) * (size_t)graph->num_vertices);
  int64_t *previous =
      (int64_t *)malloc(sizeof(int64_t) * (size_t)graph->num_vertices);
  bool *visited = (bool *)calloc((size_t)graph->num_vertices, sizeof(bool));
  ...
  for (int64_t count = 0; count < graph->num_vertices; count++) {
    ...
    for (int64_t i = 0; i < neighbors->length; i++) {
      ...
      if (edge_val.type == NAMI_TYPE_OBJECT) {
        nami_value_t to_val = nami_object_get(edge_val.value.as_object, "to");
        nami_value_t weight_val =
            nami_object_get(edge_val.value.as_object, "weight");

        if (to_val.type == NAMI_TYPE_INT &&
            weight_val.type == NAMI_TYPE_FLOAT) {
          int64_t neighbor = to_val.value.as_int;
 ...
 */
static inline nami_array_t *nami_graph_astar(nami_graph_t *graph, int64_t start,
                                             int64_t end,
                                             nami_function_t heuristic) {
  if (start < 0 || start >= graph->num_vertices || end < 0 ||
      end >= graph->num_vertices) {
  ...
  bool *in_closed_set =
      (bool *)calloc((size_t)graph->num_vertices, sizeof(bool));
...
static inline nami_value_t nami_typeof(nami_value_t v) {
  ...
  case NAMI_TYPE_INT:
  case NAMI_TYPE_FLOAT:
    return nami_value_string("number");
  case NAMI_TYPE_STRING:
    return nami_value_string("string");
  case NAMI_TYPE_BOOL:
    return nami_value_string("boolean");
  case NAMI_TYPE_NULL:
    return nami_value_string("null");
  case NAMI_TYPE_ARRAY:
    return nami_value_string("array");
  case NAMI_TYPE_OBJECT:
    return nami_value_string("object");
  case NAMI_TYPE_FUNCTION:
    return nami_value_string("function");
  default:
    return nami_value_string("undefined");
  }
}
...
static inline nami_value_t nami_length(nami_value_t v) {
  switch (v.type) {
  case NAMI_TYPE_STRING:
    return nami_value_int((int64_t)v.value.as_string->length);
  case NAMI_TYPE_ARRAY:
    return nami_value_int(v.value.as_array->length);
  default:
 ...
 */
static inline nami_error_t *nami_error_create(const char *message,
                                              nami_error_type_t type,
                                              const char *file, int line,
                                              const char *function) {
...
static inline const char *nami_error_type_name(nami_error_type_t type) {
  switch (type) {
  case NAMI_ERROR_GENERIC:
    return "Error";
  case NAMI_ERROR_TYPE:
    return "TypeError";
  case NAMI_ERROR_REFERENCE:
    return "ReferenceError";
  case NAMI_ERROR_SYNTAX:
    return "SyntaxError";
  case NAMI_ERROR_RANGE:
    return "RangeError";
  case NAMI_ERROR_NULL_POINTER:
    return "NullPointerError";
  case NAMI_ERROR_DIVISION_BY_ZERO:
    return "DivisionByZeroError";
  case NAMI_ERROR_OUT_OF_MEMORY:
    return "OutOfMemoryError";
  case NAMI_ERROR_IO:
    return "IOError";
  case NAMI_ERROR_ASSERTION:
 ...
 * @return 0 on first call (entering try block), non-zero when returning from
 * longjmp (error caught)
 */
...
static inline void nami_throw(const char *message) {
  nami_error_t *error =
      nami_error_create(message, NAMI_ERROR_GENERIC, NULL, 0, NULL);
...
#define NAMI_THROW(message, type)                                              \
  nami_throw_error(                                                            \
      nami_error_create(message, type, __FILE__, __LINE__, __func__))
...
static inline void nami_throw_value(nami_value_t error_val) {
  ...
  case NAMI_TYPE_STRING:
    ...
    break;
  case NAMI_TYPE_INT:
    snprintf(message, sizeof(message), "Error code: %lld",
             (long long)error_val.value.as_int);
    nami_throw(message);
    break;
  case NAMI_TYPE_FLOAT:
    snprintf(message, sizeof(message), "Error code: %g",
             error_val.value.as_float);
    nami_throw(message);
    break;
  default:
...
static inline nami_value_t nami_parse_int(nami_value_t v) {
  if (v.type == NAMI_TYPE_INT)
    return v;
  if (v.type == NAMI_TYPE_FLOAT)
    return nami_value_int((int64_t)v.value.as_float);
  if (v.type == NAMI_TYPE_STRING)
    return nami_value_int(atoll(v.value.as_string->data));
  return nami_value_int(0);
}
...
static inline nami_value_t nami_parse_float(nami_value_t v) {
  if (v.type == NAMI_TYPE_FLOAT)
    return v;
  if (v.type == NAMI_TYPE_INT)
    return nami_value_float((double)v.value.as_int);
  if (v.type == NAMI_TYPE_STRING)
    return nami_value_float(atof(v.value.as_string->data));
...
static inline void nami_set(nami_value_t obj, nami_value_t key,
                            nami_value_t value) {
  if (obj.type == NAMI_TYPE_ARRAY && key.type == NAMI_TYPE_INT) {
...
static inline void nami_set_prop(nami_value_t obj, const char *key,
                                 nami_value_t value) {
  if (obj.type == NAMI_TYPE_OBJECT) {
...
// In C, variadic functions must fetch typed arguments. NAMI arguments are
// nami_value_t. Because we don't pass 'count' from the codegen currently the
// codegen just generates nami_method_call(obj, "method", arg1, arg2...) This is
// actually invalid standard C without a count, unless it's a macro or we assume
// a fixed number. Since codegen supports chaining and arbitrary arguments, we
// need to modify codegen.ts to pass 'count'. WAIT: Actually, we can just define
// a few overloaded macros if we're careful, but standard C doesn't support
// generic vararg macros without __VA_ARGS_OPT__ or similar. I will temporarily
// add a nami_method_call that delegates to a variadic implementation, but it
// will require tweaking how it is called, or we will just use
// nami_invoke_method(obj, "method", count, ...) and change codegen.

static inline nami_value_t
nami_invoke_method(nami_value_t obj, const char *method, int argc, ...) {
  nami_value_t func = NAMI_NULL;
  if (obj.type == NAMI_TYPE_OBJECT) {
    func = nami_object_get(obj.value.as_object, method);
  }

  // For now, support only 0 to 4 arguments since we don't have dynamic FFI
  // calls easily Standard execution of lambda functions uses nami_value_t as
  // signature. If func is a native C function pointer, we need to cast it
  // according to argc.
  if (func.type == NAMI_TYPE_FUNCTION && func.value.as_function != NULL) {
    va_list args;
    va_start(args, argc);
    nami_value_t res = NAMI_NULL;

    if (argc == 0) {
      typedef nami_value_t (*f0_t)(void);
      f0_t f = (f0_t)func.value.as_function;
      res = f();
    } else if (argc == 1) {
      typedef nami_value_t (*f1_t)(nami_value_t);
      f1_t f = (f1_t)func.value.as_function;
      res = f(va_arg(args, nami_value_t));
    } else if (argc == 2) {
      typedef nami_value_t (*f2_t)(nami_value_t, nami_value_t);
      f2_t f = (f2_t)func.value.as_function;
      nami_value_t a0 = va_arg(args, nami_value_t);
      nami_value_t a1 = va_arg(args, nami_value_t);
      res = f(a0, a1);
    } else if (argc == 3) {
      typedef nami_value_t (*f3_t)(nami_value_t, nami_value_t, nami_value_t);
      f3_t f = (f3_t)func.value.as_function;
      nami_value_t a0 = va_arg(args, nami_value_t);
      nami_value_t a1 = va_arg(args, nami_value_t);
      nami_value_t a2 = va_arg(args, nami_value_t);
      res = f(a0, a1, a2);
    } else if (argc == 4) {
      typedef nami_value_t (*f4_t)(nami_value_t, nami_value_t, nami_value_t,
                                   nami_value_t);
      f4_t f = (f4_t)func.value.as_function;
      nami_value_t a0 = va_arg(args, nami_value_t);
      nami_value_t a1 = va_arg(args, nami_value_t);
      nami_value_t a2 = va_arg(args, nami_value_t);
      nami_value_t a3 = va_arg(args, nami_value_t);
      res = f(a0, a1, a2, a3);
    }
    va_end(args);

    // Method chaining: if method returns NULL (or not set), return the object
    // itself to allow chaining. Wait, standard NAMI HTTP design uses return
    // this; explicitly, or maybe we just return the object? Let's rely on the
    // lambda's return value. Actually for method chaining like
    // req.status(200).json(), if status() returns the res object it works.
    return res;
  }
  return NAMI_NULL;
}

static inline nami_value_t nami_invoke_method_dynamic(nami_value_t obj,
                                                      nami_value_t method_val,
                                                      int argc, ...) {
  if (method_val.type == NAMI_TYPE_STRING &&
      method_val.value.as_string != NULL) {
    // Just forward to nami_invoke_method, we need a separate va_list handling
    nami_value_t func = NAMI_NULL;
    if (obj.type == NAMI_TYPE_OBJECT) {
      func = nami_object_get(obj.value.as_object,
                             method_val.value.as_string->data);
    }
 ...
 */
static inline int64_t nami_partition(nami_array_t *arr, int64_t low,
                                     int64_t high, nami_comparator_t cmp) {
 ...
 */
static inline void nami_quicksort_helper(nami_array_t *arr, int64_t low,
                                         int64_t high, nami_comparator_t cmp) {
  if (low < high) {
 ...
 */
static inline void nami_quicksort(nami_array_t *arr,
                                  nami_comparator_t comparator) {
 ...
 */
static inline void nami_merge(nami_array_t *arr, int64_t left, int64_t mid,
                              int64_t right, nami_comparator_t cmp) {
 ...
 */
static inline void nami_mergesort_helper(nami_array_t *arr, int64_t left,
                                         int64_t right, nami_comparator_t cmp) {
  if (left < right) {
 ...
 * @param comparator Custom comparison function (NULL for default)
 */
static inline void nami_mergesort(nami_array_t *arr,
                                  nami_comparator_t comparator) {
  if (arr == NULL || arr->length <= 1)
    return;
 ...
 */
static inline void nami_heapify(nami_array_t *arr, int64_t n, int64_t i,
                                nami_comparator_t cmp) {
  int64_t largest = i;
  int64_t left = 2 * i + 1;
  int64_t right = 2 * i + 2;
 ...
 * @param comparator Custom comparison function (NULL for default)
 */
static inline void nami_heapsort(nami_array_t *arr,
                                 nami_comparator_t comparator) {
  if (arr == NULL || arr->length <= 1)
    return;
 ...
 *
 * Supports sorting arrays of any comparable type through nami_to_number
 * conversion.
 *
 * @param arr Array to sort (modified in-place)
 */
static inline void nami_sort(nami_array_t *arr) { nami_quicksort(arr, NULL); }
 ...
 */
static inline nami_tree_node_t *nami_tree_insert_helper(nami_tree_node_t *node,
                                                        nami_value_t value) {
 ...
 */
static inline bool nami_tree_search_helper(nami_tree_node_t *node,
                                           nami_value_t value) {
  if (node == NULL)
    return false;
 ...
 */
static inline void nami_tree_inorder_helper(nami_tree_node_t *node,
                                            nami_array_t *result) {
  if (node == NULL)
    return;
 ...
 * @param result Array to append values to
 */
static inline void nami_tree_preorder_helper(nami_tree_node_t *node,
                                             nami_array_t *result) {
  if (node == NULL)
    return;
 ...
 * @param result Array to append values to
 */
static inline void nami_tree_postorder_helper(nami_tree_node_t *node,
                                              nami_array_t *result) {
  if (node == NULL)
    return;
...
typedef struct nami_promise {
  ...
  void (*continuation)(
      void *);   // Continuation callback to execute when promise resolves
 ...
 */
static inline void nami_async_resolve(nami_promise_t *promise,
                                      nami_value_t value) {
  if (promise == NULL)
    return;
 ...
 */
static inline void nami_async_reject(nami_promise_t *promise,
                                     nami_value_t error) {
  if (promise == NULL)
    return;
...
static inline nami_value_t nami_async_await(nami_promise_t *promise) {
  ...
  // The code generator should use state machines instead of calling this
  // directly.
 ...
 */
static inline void nami_async_then(nami_promise_t *promise,
 ...
 */
static inline void nami_pointer_register(void *ptr, size_t size,
                                         bool is_nullable) {
  if (!__nami_pointer_registry.enabled || ptr == NULL)
    return;

  nami_pointer_metadata_t *metadata =
      (nami_pointer_metadata_t *)malloc(sizeof(nami_pointer_metadata_t));
 ...
 */
static inline void nami_pointer_check_bounds(void *ptr, size_t access_size,
                                             const char *file, int line) {
  ...
  if (metadata == NULL) {
    fprintf(stderr,
            "Error: Pointer bounds check failed - pointer not tracked\n");
    fprintf(stderr, "  Pointer: %p\n", ptr);
    if (file != NULL) {
      fprintf(stderr, "  Location: %s:%d\n", file, line);
    }
    fprintf(stderr, "  Note: This pointer may not have been allocated through "
                    "tracked allocation\n");
    exit(1);
  }
 ...
 * @param line Line number where the check is performed (for error reporting)
 */
static inline void nami_pointer_check_null(void *ptr, bool is_nullable,
                                           const char *file, int line) {
  if (!__nami_pointer_registry.enabled)
    return;
  ...
  if (ptr == NULL) {
    fprintf(stderr,
            "Error: Null pointer dereference on non-nullable pointer\n");
    if (file != NULL) {
      fprintf(stderr, "  Location: %s:%d\n", file, line);
    }
...
#define NAMI_CHECK_BOUNDS(ptr, size)                                           \
  nami_pointer_check_bounds(ptr, size, __FILE__, __LINE__)
#define NAMI_CHECK_NULL(ptr, is_nullable)                                      \
  nami_pointer_check_null(ptr, is_nullable, __FILE__, __LINE__)

// ── HTTP Module ─────────────────────────────────────────
// Include HTTP implementation
// Priority: Simple > Mongoose > Stub
#if defined(NAMI_USE_MONGOOSE)
#include "nami_http_mongoose.h"
#elif defined(NAMI_USE_SIMPLE_HTTP) || !defined(NAMI_HTTP_STUB_ONLY)
#include "nami_http_simple.h"
#else
#include "nami_http_stub.h"
#endif
