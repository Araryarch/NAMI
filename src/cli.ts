...
function createProgram(): Command {
  ...
  program
    .command('run')
    .description('Compile and run a NAMI program (like node/python)')
    .argument('<file>', 'NAMI source file (.nm)')
    .option('-O, --optimization <level>', 'Optimization level (debug, release, max)', 'debug')
    .option('--with-mongoose', 'Enable Mongoose HTTP server support', false)
    .option('--keep-temp', 'Keep temporary build files', false)
    .action((file: string, options: CompileOptions & { withMongoose?: boolean; keepTemp?: boolean }) => {
      if (!fs.existsSync(file)) {
        console.error(`\x1b[31mError: File not found: ${file}\x1b[0m`);
        process.exit(1);
      }
      ...
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'nami-run-'));
      const baseName = path.basename(file, path.extname(file));
      const sourceFile = path.join(tmpDir, `${baseName}.c`);
      const tmpExec = path.join(tmpDir, baseName);
      ...
      try {
        ...
        const runtimeHeader = path.join(__dirname, '..', 'runtime', 'nami_runtime.h');
        if (fs.existsSync(runtimeHeader)) {
          fs.copyFileSync(runtimeHeader, path.join(tmpDir, 'nami_runtime.h'));
        }

        const httpStub = path.join(__dirname, '..', 'runtime', 'nami_http_stub.h');
        if (fs.existsSync(httpStub)) {
          fs.copyFileSync(httpStub, path.join(tmpDir, 'nami_http_stub.h'));
        }

        const httpMongoose = path.join(__dirname, '..', 'runtime', 'nami_http_mongoose.h');
        if (fs.existsSync(httpMongoose)) {
          fs.copyFileSync(httpMongoose, path.join(tmpDir, 'nami_http_mongoose.h'));
        }

        // Copy mongoose if requested
        if (options.withMongoose) {
          const mongooseH = path.join(__dirname, '..', 'runtime', 'mongoose.h');
          const mongooseC = path.join(__dirname, '..', 'runtime', 'mongoose.c');
          if (fs.existsSync(mongooseH) && fs.existsSync(mongooseC)) {
            fs.copyFileSync(mongooseH, path.join(tmpDir, 'mongoose.h'));
            fs.copyFileSync(mongooseC, path.join(tmpDir, 'mongoose.c'));
          } else {
            console.error('\x1b[33mWarning: Mongoose not found. Run ./download-mongoose.sh first\x1b[0m');
          }
        }

        const compiler = findCCompiler();
        if (!compiler) {
          console.error('\x1b[31mError: No C compiler found\x1b[0m');
          process.exit(1);
        }

        // Build compiler flags
        let optFlags = options.optimization === 'release' ? '-O2' : '-g';
        let linkFlags = '-lm';
        let extraFiles = '';

        if (options.withMongoose) {
          const mongooseC = path.join(tmpDir, 'mongoose.c');
          if (fs.existsSync(mongooseC)) {
            extraFiles = `"${mongooseC}"`;
            linkFlags += ' -lpthread';
            optFlags += ' -DNAMI_USE_MONGOOSE';
          }
        }

        const command = `${compiler} ${optFlags} -o "${tmpExec}" "${sourceFile}" ${extraFiles} -I"${tmpDir}" ${linkFlags}`;
        ...
        execSync(`"${tmpExec}"`, { stdio: 'inherit' });
        console.log('\n\x1b[2m─────────────────────────────────────\x1b[0m');
        ...
        if (error.status !== undefined) {
          ...
          console.error('\x1b[31mError: Compilation failed\x1b[0m');
          if (error.stderr) {
            console.error(error.stderr.toString());
          }
          process.exit(1);
