...
import { Program, ImportStatement, NamiImportStatement } from '../parser/ast';
...
export function extractImports(ast: Program): string[] {
  ...
  for (const statement of ast.body) {
    if (statement.type === 'ImportStatement') {
      ...
      const namiImport = statement as NamiImportStatement;
...
export function generateModuleFiles(graph: ModuleDependencyGraph): Map<string, ModuleCodeGeneration> {
...
export function generateIncludeDirectives(node: ModuleNode, graph: ModuleDependencyGraph): string[] {
