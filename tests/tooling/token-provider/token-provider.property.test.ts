...
const namiBlockComment = fc.string({ maxLength: 100 }).map(s => `/* ${s.replace(/\*\//g, '').replace(/\/\*/g, '')} */`);
...
const namiIfStatement = fc.tuple(
...
const namiLoopStatement = fc.tuple(
  fc.array(namiStatement, { minLength: 1, maxLength: 3 })
).map(([body]) => `loop {\n  ${body.join('\n  ')}\n}`);
...
const namiCodeWithComments = fc.tuple(
  namiProgram,
  fc.array(namiComment, { maxLength: 3 })
).map(([code, comments]) => {
...
describe('Property 19: Token Provider Position Accuracy', () => {
  ...
  it('should maintain position consistency across multi-line code', () => {
    fc.assert(
      fc.property(
        fc.array(namiStatement, { minLength: 2, maxLength: 10 }),
        (statements) => {
